...
const API_URL = `${API_BASE}/linq/watchlist/?all=true`;
...
export default function AdminPage() {
  ...
  async function sendDiscountAlert(item) {
    ...
    setActionError("");
    try {
      const response = await fetch(`${API_BASE}/linq/send-message/`, {
        method: "POST",
      ...
      if (!response.ok) {
        ...
        throw new Error(
          result.error || `Notification failed (${response.status})`,
        );
      }
      setSentIds((current) => ({ ...current, [item.product_id]: true }));
    } catch (requestError) {
      setActionError(
        requestError.message || "Could not send the discount alert.",
      );
    } finally {
      setSendingId("");
    }
  ...
  return (
    <main className="admin-page">
      ...
      {status === "loading" && (
        <p className="admin-page__message">Loading watchlists...</p>
      )}
      {status === "error" && (
      ...
      {status === "ready" && items.length === 0 && (
        <p className="admin-page__message">
          No watchlist items have been saved yet.
        </p>
      )}
      {actionError && (
        <p className="admin-page__message admin-page__message--error">
      ...
      {status === "ready" && items.length > 0 && (
        <section
          className="admin-watchlist-grid"
          aria-label="All watchlist items"
        >
          {items.map((item) => (
            <article
              className="admin-watchlist-card"
              key={`${item.owner_number}-${item.product_id}`}
            >
              ...
              <div className="admin-watchlist-card__body">
                <span className="admin-watchlist-card__brand">
                  {item.brand || item.merchant || "Unbranded"}
                </span>
                <h2>{item.name}</h2>
                ...
                <div className="admin-watchlist-card__alert">
                  <label htmlFor={`new-price-${item.product_id}`}>
                    New price
                  </label>
                  <div className="admin-watchlist-card__alert-controls">
                    ...
                    >
                      {sendingId === item.product_id
                        ? "Sending..."
                        : "Save changes"}
                    </button>
                  </div>
                  {sentIds[item.product_id] && (
                    <span className="admin-watchlist-card__sent">
                      Alert sent
                    </span>
                  )}
                </div>
                <dl>
                  <div>
                    <dt>Owner</dt>
                    <dd>{item.owner_number}</dd>
                  </div>
                  <div>
                    <dt>Variant</dt>
                    <dd>{item.variant || "Not specified"}</dd>
                  </div>
                  <div>
                    <dt>Status</dt>
