...
class App {
  ...
  onTouchDown(e) {
    ...
    this.clickStart = e.touches ? e.touches[0].clientX : e.clientX;
  }
  onTouchMove(e) {
  ...
  onTouchUp(e) {
    ...
    if (Math.abs(this.clickStart - end) < 10) {
      if (this.onItemClick) {
        // approximate which item is in the center
        const width = this.medias[0].width;
        const itemIndex = Math.round(Math.abs(this.scroll.target) / width);
        const originalIndex = itemIndex % (this.medias.length / 2); // since we duplicate
