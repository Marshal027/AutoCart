...
export default function SpecularButton({
  ...
  useEffect(() => {
    ...
    if (!button || !effect || typeof window === "undefined") return undefined;
    ...
    try {
      ...
      const update = (now) => {
        ...
        const steer =
          props.followMouse &&
          pointerAngle !== null &&
          (!props.autoAnimate || proximityT > 0);
        const target = steer ? pointerAngle : idleAngle;
        const diff = ((target - angle + Math.PI * 3) % (Math.PI * 2)) - Math.PI;
        angle += diff * (1 - Math.exp(-dt * 7));
        const brightnessTarget = props.autoAnimate ? 1 : proximityT;
        brightness += (brightnessTarget - brightness) * (1 - Math.exp(-dt * 8));
      ...
      // The CSS button remains usable if WebGL is unavailable.
      return undefined;
    }
  }, []);
