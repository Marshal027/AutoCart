...
import { 
  Sparkles, Tag, Clock, CreditCard, Ticket, Zap, Truck, Repeat, 
  ArrowRight, CheckCircle2, AlertCircle, Search, Filter, Bell, X, 
  ChevronDown, ArrowUpRight, TrendingUp, Info
} from 'lucide-react';
...
const TypeIcon = ({ type }: { type: string }) => {
  switch (type) {
    case 'SALE': return <Tag className="h-4 w-4 text-accent" />;
    case 'BANK_OFFER': return <CreditCard className="h-4 w-4 text-accent-orange" />;
    case 'COUPON': return <Ticket className="h-4 w-4 text-accent" />;
    case 'FLASH_DEAL': return <Zap className="h-4 w-4 text-accent-orange" />;
    case 'FREE_SHIPPING': return <Truck className="h-4 w-4 text-accent" />;
    case 'CASHBACK': return <CreditCard className="h-4 w-4 text-accent" />;
    case 'EXCHANGE_OFFER': return <Repeat className="h-4 w-4 text-accent-orange" />;
    default: return <Sparkles className="h-4 w-4 text-accent" />;
  }
};
...
export const SmartSavings = ({ currentGoal }: { currentGoal?: string }) => {
  ...
  useEffect(() => {
    ...
    const fetchSearch = async () => {
      setLoading(true);
      try {
        const response = await postJson<SearchResponse>('/opportunities/search/', {
          query: searchQuery,
          goal: currentGoal,
          filters: {
  ...
  }, [searchQuery, activeCategory, selectedStore, selectedType, selectedStatus, sortBy, currentGoal]);
  ...
  const getScoreColor = (score: number) => {
    if (score >= 80) return 'text-accent border-accent bg-accent/10 shadow-[0_0_15px_rgba(0,255,153,0.3)]';
    if (score >= 50) return 'text-accent-orange border-accent-orange bg-accent-orange/10 shadow-[0_0_10px_rgba(255,153,0,0.2)]';
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="relative z-30" ref={searchRef}>
        <div className="flex items-center border border-border-col bg-bg hover:border-accent transition-colors px-4 py-3 relative">
          ...
          />
          {loading && <div className="h-4 w-4 border-2 border-accent border-t-transparent rounded-full animate-spin ml-3" />}
        </div>
              ...
              <div className="flex flex-col gap-2">
                {trending.map((t, idx) => (
                  <button 
                    key={idx}
                    type="button"
                    onClick={() => { setSearchQuery(t); setShowAutocomplete(false); }}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select value={selectedType} onChange={e => setSelectedType(e.target.value)}
          ...
          {filterOptions?.types.map(t => <option key={t} value={t}>{t.replace('_', ' ')}</option>)}
        </select>
      ...
      {data.length === 0 && !loading ? (
        <div className="py-20 text-center border border-border-col border-dashed">
          ...
          <h3 className="font-['Oswald'] text-xl uppercase tracking-widest text-text/50">No opportunities found</h3>
          <p className="font-['Space_Mono'] text-sm text-text/40 mt-2">Try adjusting your filters or search query.</p>
        </div>
      ) : (
            ...
            >
              <div>
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="relative">
                    <button 
                      onClick={() => setExplainPopoverId(explainPopoverId === opp.id ? null : opp.id)}
                    ...
                    >
                      <span className="text-xs font-bold font-['Space_Mono']">{opp.ai_score}</span>
                    </button>
                      ...
                      {explainPopoverId === opp.id && (
                        ...
                        >
                          <div className="flex items-center gap-2 mb-2 text-accent border-b border-accent/20 pb-2">
                            ...
                            <span className="text-[10px] font-bold uppercase font-['Space_Mono'] tracking-widest">AI Score Info</span>
                          </div>
                ...
                <div className="space-y-2 mb-4">
                  ...
                  {opp.coupon && (
                    <div className="flex items-center justify-between text-sm text-text/70 bg-bg/50 p-2 border border-border-col/50">
                      ...
                      <span className="font-['Space_Mono'] font-bold text-accent uppercase tracking-widest px-2 py-0.5 bg-accent/10 border border-accent/30">{opp.coupon}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-border-col flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 relative">
                  {/* Watch Button */}
                  <button 
                    onClick={() => setWatchPopoverId(watchPopoverId === opp.id ? null : opp.id)}
                    ...
                    {watchPopoverId === opp.id && (
                      ...
                      >
                        <div className="flex items-center justify-between mb-3 pb-2 border-b border-border-col">
                          ...
                          <button onClick={() => setWatchPopoverId(null)} className="text-text/50 hover:text-text"><X className="h-4 w-4"/></button>
                        </div>
                        <div className="space-y-2 mb-4">
                          {Object.entries(watchPrefs).map(([key, val]) => (
                            <label key={key} className="flex items-center gap-2 cursor-pointer group">
                              <input 
                                type="checkbox" 
                                checked={val}
                                onChange={() => setWatchPrefs({...watchPrefs, [key]: !val})}
                              ...
                              <span className="text-xs font-['Space_Mono'] text-text/70 group-hover:text-text transition-colors">
                                {key.replace('notify_', '').replace(/_/g, ' ').toUpperCase()}
                              </span>
                            </label>
                          ))}
                        </div>
