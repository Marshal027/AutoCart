...
const TargetCursor = ({
  ...
  useEffect(() => {
    ...
    const enterHandler = e => {
      ...
      // If we are hiding default cursor conditionally, 
      // when over a target, ensure target element has cursor: none
      if (target && !hideDefaultCursor) {
        target.style.cursor = 'none';
        gsap.to(cursorRef.current, { opacity: 1, duration: 0.2 }); // Fade cursor in when over target if conditionally hidden globally
      }
      ...
      const leaveHandler = () => {
        if (!hideDefaultCursor && cursorRef.current) {
           gsap.to(cursorRef.current, { opacity: 0, duration: 0.2 }); // fade out cursor when leaving target
        }
