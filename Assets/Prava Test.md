...
## Test this repository locally

1. Create `backend/.env` from `backend/.env.example`.
2. Replace the placeholder with your real `sk_test_*` key from the Prava dashboard.
3. Start Django from `backend/`:
...
5. Open `http://localhost:5173/prava` and click **Pay with Prava Sandbox**.
6. Complete the hosted Prava sandbox flow with a test card and passkey.

The main Autocart flow uses the same backend credentials. Add MCP products to the
cart, click **Proceed to pay**, and Autocart will create one session per MCP merchant
through `POST /api/prava/sessions/`.

Never put `MERCHANT_SECRET_KEY` in frontend code or a `VITE_*` variable.
